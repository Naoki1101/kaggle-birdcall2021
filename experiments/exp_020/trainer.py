...
class NNTrainer:
    ...
    def train(
        ...
        for epoch in mb:
            ...
            events = valid_preds >= self.cfg.threshold
            nocall_col = np.zeros((len(valid_preds), 1)).astype(bool)
            nocall_col[events.sum(1) == 0] = True
            events = np.concatenate([events, nocall_col], axis=1)
            valid_preds_labels = list(
                map(
            ...
            val_score = scorer(val_y_labels, valid_preds_labels)
    ...
    def _train_epoch(self, model, train_loader, criterion, optimizer, mb):
        ...
        for i, (feats, targets) in enumerate(progress_bar(train_loader, parent=mb)):
            ...
            if self.cfg.data.train.mixup and r < 0.5:
                feats, targets = mixup(feats, targets, 1.0)
