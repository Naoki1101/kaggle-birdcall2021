...
class NNTrainer:
    ...
    def train(
        ...
        valid_target_df = valid_target_df.iloc[use_cv_idx].reset_index(drop=True)
        ...
        train_loader = get_dataloader(train_df, target_df, self.cfg.data.train)
        valid_loader = get_dataloader(valid_df, valid_target_df, self.cfg.data.valid)
        ...
        for epoch in mb:
            ...
            valid_pred_labels = self._extract_label(valid_preds)

            val_y_labels = valid_df["birds"].tolist()
            val_score = scorer(val_y_labels, valid_pred_labels)
            cor_score = scorer(val_y_labels[:1_200], valid_pred_labels[:1_200])
            ssw_score = scorer(val_y_labels[1_200:], valid_pred_labels[1_200:])
            ...
            mb.write(
                f"Epoch {epoch+1} - avg_train_loss: {avg_loss:.6f}  avg_val_loss: {avg_val_loss:.6f} val_score: {val_score:.6f} COR: {cor_score:.6f} SSW: {ssw_score:.6f} time: {elapsed:.0f}s"
            )
            logging.debug(
                f"Epoch {epoch+1} - avg_train_loss: {avg_loss:.6f}  avg_val_loss: {avg_val_loss:.6f} val_score: {val_score:.6f} COR: {cor_score:.6f} SSW: {ssw_score:.6f} time: {elapsed:.0f}s"
            )
        ...
        torch.save(best_model, const.LOG_DIR / f"{self.run_name}/weight_best.pt")
    ...
    def _extract_label(self, preds):
        events = preds >= self.cfg.threshold
    ...
    def _train_epoch(self, model, train_loader, criterion, optimizer, mb):
        ...
        for i, (images, targets) in enumerate(progress_bar(train_loader, parent=mb)):
            ...
            if self.cfg.data.train.mixup and r < 0.5:
                feats, targets = mixup(images, targets, 1.0)
...
def mixup(images, targets, alpha):
    indices1 = torch.randperm(images.size(0))
    indices2 = torch.randperm(images.size(0))

    shuffled_images1 = images[indices1]
    shuffled_targets1 = targets[indices1]

    shuffled_images2 = images[indices2]
    shuffled_targets2 = targets[indices2]

    lam = np.random.beta(alpha, alpha, 3)
    lam /= lam.sum()

    images = images * lam[0] + shuffled_images1 * lam[1] + shuffled_images2 * lam[2]

    targets += shuffled_targets1 + shuffled_targets2
