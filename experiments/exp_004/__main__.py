...
def main():
    ...
    with t.timer("preprocess"):
        ...
        for idx in train_df.index:
            secondary_labels_str = train_df.loc[idx, "secondary_labels"]
            secondary_labels = re.findall(p, secondary_labels_str)

            if len(secondary_labels) > 0:
                for secondary_label in secondary_labels:
                    if secondary_label in const.BIRD_CODE:
                        col_idx = const.BIRD_CODE[secondary_label]
                        target_df.loc[idx, col_idx] = min(
                            target_df.loc[idx, col_idx] + 0.3, 1
                        )
