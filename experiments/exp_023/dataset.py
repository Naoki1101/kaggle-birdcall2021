...
class CustomTrainDataset(Dataset):
    def __init__(self, df: pd.DataFrame, target_df: pd.DataFrame, cfg):
        ...
        df["latitude"] = (df["latitude"].values + 90.0) / 180.0
        df["longitude"] = (df["longitude"].values + 180.0) / 360.0
        self.position = df[["latitude", "longitude"]].values
...
class CustomValidDataset(Dataset):
    def __init__(self, df: pd.DataFrame, target_df: pd.DataFrame, cfg):
        ...
        position_df = pd.DataFrame.from_dict(const.POS_DICT, orient="index")
        df["latitude"] = (df["site"].map(position_df["latitude"]) + 90.0) / 180.0
        df["longitude"] = (df["site"].map(position_df["longitude"]) + 180) / 360.0
        self.position = df[["latitude", "longitude"]].values
