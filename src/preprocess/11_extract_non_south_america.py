...
def main():
    ...
    count_df["count_non_south_america"] = count_df["primary_label"].map(other_ce)

    count_df["percent_other_regions"] = (
        count_df["count_non_south_america"] / count_df["count_all_region"]
    )
    ...
    np.save(const.PROCESSED_DATA_DIR / "non_south_america_idx.npy", drop_idx)
