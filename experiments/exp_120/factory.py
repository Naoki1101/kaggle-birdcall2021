...
from .dataset import CustomTrainDataset, CustomValidDataset
...
def get_loss(cfg, pos_weight=None):
    ...
    if hasattr(nn, cfg.loss.name):
        loss_ = getattr(nn, cfg.loss.name)(pos_weight=pos_weight, **cfg.loss.params)
    elif hasattr(loss, cfg.loss.name):
        loss_ = getattr(loss, cfg.loss.name)(pos_weight=pos_weight, **cfg.loss.params)
    return loss_
