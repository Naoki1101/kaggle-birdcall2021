...
def main():
    ...
    with t.timer("preprocess"):
        ...
        min_distance_array = np.load(
            const.PROCESSED_DATA_DIR / "distance_array_from_min_position.npy"
        )

        gp = train_df.groupby("primary_label")
        for pl, bird_df in gp:
            idxs = bird_df.index.values
            bird_distance_array = min_distance_array[idxs, :].mean(0)

            whole_target_array[idxs, :-1] += np.where(bird_distance_array <= 5, 0.1, 0)
            whole_target_array[idxs, :] = np.clip(whole_target_array[idxs, :], 0.0, 1.0)
