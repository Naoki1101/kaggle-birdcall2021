...
class NNTrainer:
    ...
    def train(
        ...
        for fold_, col in enumerate(self.folds):
            print(
                f"\n========================== FOLD {fold_ + 1} / {len(self.folds)} ... ==========================\n"
            )
            logging.debug(
                f"\n========================== FOLD {fold_ + 1} / {len(self.folds)} ... ==========================\n"
            )
            ...
            trn_x = train_df[fold_df[col] == 0]
            trn_y = target_df[fold_df[col] == 0]

            train_loader = get_dataloader(trn_x, trn_y, self.cfg.data.train)
            valid_loader = get_dataloader(
                valid_df, valid_target_df, self.cfg.data.valid
            )
            ...
            torch.save(
                best_model, const.LOG_DIR / f"{self.run_name}/weight_best{fold_}.pt"
            )
