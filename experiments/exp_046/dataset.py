...
class CustomTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if noise_r >= 0.95:
            noise_name = np.random.choice(list(self.noise_dict.keys())[1:])
            noise_wave = self.noise_dict[noise_name]
            start = np.random.randint(len(noise_wave) - self.samples)
            y = noise_wave[start : start + self.samples].astype(np.float32)

            label = np.zeros(len(const.BIRD_CODE)).astype(float)
            label[-1] = 1.0
            ...
            if self.cfg.noise:
                ...
                for noise_name in self.cfg.noise:
                    ...
                    if rand >= noise_threshhold[0] and rand < noise_threshhold[1]:
                        start = np.random.randint(len(noise_wave) - self.samples)
                        y += (
                            noise_wave[start : start + self.samples].astype(np.float32)
                            * m
                        )
