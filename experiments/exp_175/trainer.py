...
class NNTrainer:
    ...
    def train(
        ...
        for epoch in mb:
            ...
            valid_pred_labels, th = self._optim_th(valid_preds, scorer, val_y_labels)
            ...
            mb.write(
                f"Epoch {epoch+1} - avg_train_loss: {avg_loss:.6f}  avg_val_loss: {avg_val_loss:.6f} val_score: {val_score:.6f} COR: {cor_score:.6f} SSW: {ssw_score:.6f} th: {th} time: {elapsed:.0f}s"
            )
            logging.debug(
                f"Epoch {epoch+1} - avg_train_loss: {avg_loss:.6f}  avg_val_loss: {avg_val_loss:.6f} val_score: {val_score:.6f} COR: {cor_score:.6f} SSW: {ssw_score:.6f} th: {th} time: {elapsed:.0f}s"
            )
    ...
    def _extract_label(self, preds, threshold):
    ...
    def _optim_th(self, preds, scorer, y_true_label):
        best_score = 0
        best_th = 0
        best_labels = None
        ...
        for i in range(20, 70):
            ...
            pred_labels = self._extract_label(preds, th)
            score = scorer(y_true_label, pred_labels)
