...
def main():
    ...
    with t.timer("preprocess"):
        ...
        smoothing_bird_idxs = np.argsort(min_distance_array, axis=1)[:, :10]
        for idx, col_idxs in enumerate(smoothing_bird_idxs):
            whole_target_array[idx, col_idxs] += 0.1

        whole_target_array = np.clip(whole_target_array, 0.0, 1.0)
