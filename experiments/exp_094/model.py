...
import timm
from timm.models.layers.linear import Linear
...
class CustomModel(nn.Module):
    def __init__(
        ...
        final_children = list(self.backbone.children())[-1]
        if type(final_children) == Linear:
            final_in_features = final_children.in_features
            self.backbone = nn.Sequential(*list(self.backbone.children())[:-2])
        else:
            final_in_features = list(final_children.children())[-1].in_features
