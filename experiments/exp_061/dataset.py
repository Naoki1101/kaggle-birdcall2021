...
class CustomTrainDataset(Dataset):
    def __init__(self, df: pd.DataFrame, target_df: pd.DataFrame, cfg):
        ...
        self.samples = const.TARGET_SAMPLE_RATE * self.cfg.sample_sec
...
class CustomValidDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        start_index = const.TARGET_SAMPLE_RATE * max(
            0, seconds - ((self.cfg.sample_sec + 5) / 2)
        )
        end_index = const.TARGET_SAMPLE_RATE * min(
            600, seconds + ((self.cfg.sample_sec - 5) / 2)
        )
        y = y[int(start_index) : int(end_index)].astype(np.float32)

        if len(y) <= const.TARGET_SAMPLE_RATE * self.cfg.sample_sec:
            padding = (const.TARGET_SAMPLE_RATE * self.cfg.sample_sec) - len(y)
            offset = padding // 2
            y = np.pad(
                y,
                (
                    offset,
                    (const.TARGET_SAMPLE_RATE * self.cfg.sample_sec) - len(y) - offset,
                ),
                "constant",
            ).astype(np.float32)
