...
class CustomValidDataset(Dataset):
    def __init__(self, df: pd.DataFrame, target_df: pd.DataFrame, cfg):
        ...
        position_dict = dh.load(const.INPUT_DATA_DIR / "valid_position.json")
        position_df = pd.DataFrame.from_dict(position_dict, orient="index")
        df["latitude"] = (
            df["audio_id"].astype(str).map(position_df["latitude"]) + 90.0
        ) / 180.0
        df["longitude"] = (
            df["audio_id"].astype(str).map(position_df["longitude"]) + 180
        ) / 360.0
        self.position = df[["latitude", "longitude"]].values
