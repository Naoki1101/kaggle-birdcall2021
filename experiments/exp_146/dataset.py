...
class CustomTrainDataset(Dataset):
    def __init__(self, df: pd.DataFrame, target_df: pd.DataFrame, cfg):
        ...
        self.samples = const.TARGET_SAMPLE_RATE * 7
    ...
    def __getitem__(self, idx):
        ...
        if len_y >= self.samples:
            ...
            if r < 0.9:
                ...
                if len(y) == self.samples * 2:
                    start = self.samples
                else:
                    start = np.random.randint(len(y) - self.samples * 2) + self.samples
                y = (y[: self.samples] + y[start : start + self.samples]).astype(
                    np.float32
                )
        ...
        if self.cfg.noise:
            ...
            for noise_name in self.cfg.noise:
                ...
                if rand >= noise_threshhold[0] and rand < noise_threshhold[1]:
                    ...
                    noise_wave_cropped = noise_wave[
                        start : start + self.samples
                    ].astype(np.float32)
                    noise_rms = np.sqrt(np.mean(np.square(noise_wave_cropped)))

                    y = (y * noise_rms) + (noise_wave_cropped * rms) * m
...
class CustomValidDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        start_index = const.TARGET_SAMPLE_RATE * max(0, seconds - 6)
        end_index = const.TARGET_SAMPLE_RATE * min(600, seconds + 1)
        y = y[start_index:end_index].astype(np.float32)
        y = self._padding(y, const.TARGET_SAMPLE_RATE * 7)
